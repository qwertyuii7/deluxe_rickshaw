...
export function WeatherToggle({ isRainMode, onToggle }: WeatherToggleProps) {
  return (
    <button
      onClick={onToggle}
      className={`flex items-center justify-center gap-3 px-8 py-4 rounded-full backdrop-blur-md border transition-all duration-500 shadow-[0_0_20px_rgba(0,0,0,0.5)] transform hover:scale-105 active:scale-95 ${
        isRainMode 
    ...
    >
      {isRainMode ? (
        <>
          ...
          <span className="text-xl font-medium tracking-wide">बारिश</span>
        </>
      ) : (
        <>
          <Sun size={24} className="text-rickshaw-yellow" />
          <span className="text-xl font-medium tracking-wide">मौसम साफ़</span>
        </>
      )}
    </button>
  );
